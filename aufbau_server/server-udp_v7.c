// server-v6:UDP Socket + DTLS handshake + CID + working Certs (beide Seiten)+ coap
#include <stdio.h>
#include <string.h>
...
#define ENABLE_WOLFSSL_TIMESTAMPS //Comment out to remove timestamps from debug logs
...
#ifdef ENABLE_WOLFSSL_TIMESTAMPS
...
int main()
{
    ...
    while (1)
    {
        ...
        PRINTF(GREEN "client_ip_previous: %s; client_ip_new: %s" RESET "\n", client_ip, client_ip_new);
        PRINTF(GREEN "client_port_previous: %u; client_port_new: %u" RESET "\n", client_port, client_port_new);
        ...
        if (coap_pdu_get_type(received_pdu) == COAP_MESSAGE_CON)
        {
            ...
            uint8_t ack_buffer[COAP_MAX_PDU_SIZE];
            size_t offset = 0; 
